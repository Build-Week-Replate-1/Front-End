...
import { claimFood } from "../../components/Actions/index";
...
class VolunteerDashboard extends React.Component {
  ...
  getFood = () => {
    ...
    axios
      .get("https://replated.herokuapp.com/api/requests/pending", requestConfig)
      .then(res => {
        res.data.map(data => {
  ...
  getBusinesses = () => {
    ...
    axios
      .get(
        "https://replated.herokuapp.com/api/requests/add",
        requestConfig
      )
      .then(res => {
        this.setState({
